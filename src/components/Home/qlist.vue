<template>
  <div class="qs-list">
    <ul v-if="qsList.length == 0 ? false : true">
      ...
      <li>截止时间</li>
      <li>状态</li>
      <li>操作<span @click="$router.push({name: 'qEdit', params: {num: 0}})">+新建问卷</span></li>
    </ul>
    <template v-for="item in qsList">
      <ul>
        ...
        <li>
          ...
          <router-link :to="`/fill/${item.num}`" tag="button">填写问卷</router-link>
          <button @click="iterator = watchPeople(item); iterator.next()">查看考生</button>
    ...
    <div class="list-bottom" v-if="qsList.length == 0 ? false : true">
      ...
      <div class="add-qs" v-if="qsList.length === 0">
        ...
        @click="$router.push({name: 'qEdit', params: {num: 0}})">+&nbsp;&nbsp;新建问卷</button>
      </div>
...
<script>
  export default {
    ...
    mounted() {
        this.qsList = this.$store.state.questionlist
      ...
      *edit(item) {
        ...
        yield (() => {
          if (item.state !== 'noissue') {
            ...
            this.showDialog = false;
            this.$router.push({name: 'qEdit', params: { num: item.num ,questionlist: this.$store.state.questionlist}})
          }
        })();
      },
      *watchPeople(item) {
        yield (() => {
          if (item.state === 'noissue') {
            ...
            this.$router.push({ name: 'qPeople', params: { num: item.num }})
          }
        })();
      ...
      *watchData(item) {
        yield (() => {
          if (item.state === 'noissue') {
            ...
            this.$router.push({ name: 'qData', params: { num: item.num }})
          }
        })();
        yield this.showDialog = false;
      }
    ...
    watch: {
      qslist: {
        handler(val) {
          ...
          this.$store.dispatch('savelist',val);
        },
        deep: true
      }
    }
