...
export default {
  ...
  data() {
    return {
      qsItem: {},
      qsList: this.$store.state.questionlist,
      isError: false,
      showBtn: false,
      titleChange: false,
      titleValue: '',
  ...
  beforeRouteEnter(to, from ,next) {
    ...
    let theItem = {}
    if (num != 0) {
      let length = to.params.questionlist.length
      if (num < 0 || num > length) {
        alert('非法路由!')
        next('/')
      } else {
        for (let i = 0; i < length; i++) {
          if (to.params.questionlist[i].num == num) {
            theItem = to.params.questionlist[i]
            break
          }
        }
      }
  ...
  methods: {
    ...
    *save() {
      ...
      if (this.qsItem.question.length === 0) {
        ...
        this.$store.dispatch('savelist',this.qsList)
        this.info = '是否发布?'
        this.isGoIndex = true
      }
